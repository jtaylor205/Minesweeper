...
int main()
{
    ...
    while (startWindow.isOpen()) {
        Event event;
            ...
            Writing welcomeWriting = Writing("WELCOME TO MINESWEEPER!", Color::White, 24, gameWidth / 2.0f,
                                             gameHeight / 2.0f - 150);
            Text welcomeText = welcomeWriting.getText();
            welcomeText.setStyle(Text::Bold | Text::Underlined);
            startWindow.draw(welcomeText);
            Writing nameWriting = Writing("Enter your name:", Color::White, 20, gameWidth / 2.0f,
                                          gameHeight / 2.0f - 75);
